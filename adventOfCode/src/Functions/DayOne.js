...
const calibrateNumbers = (input) => {
  const calibratedNumbers = [];
  let total = 0;
  //Split up the multiline string into it's an array of it's parts
  const unCalibratedNumbers = input.split("\n");
  let firstNumber;
  let lastNumber;

  //Idea for later, turn this into an async function so that both loops run at the same time to speed up the process
  //Loop through the elements of uncalibrated numbers
  //To show I understand looping, there are much simpler ways to do this
  for (let i = 0; i < unCalibratedNumbers.length; i++) {

    for (let j = 0; j < unCalibratedNumbers[i].length; j++) {
      if (Number(unCalibratedNumbers[i][j])) {
        firstNumber = unCalibratedNumbers[i][j];
        break;
      }
    }

    for (let k = unCalibratedNumbers[i].length - 1; k >= 0; k--) {
      if (Number(unCalibratedNumbers[i][k])) {
        lastNumber = unCalibratedNumbers[i][k];
        break;
      }
    }

      //Add numbers together and push to calibrated Numbers
      calibratedNumbers.push(firstNumber + lastNumber);
  }


  //Add numbers together
  for (let h = 0; h < calibratedNumbers.length; h++) {
    total += Number(calibratedNumbers[h])
  }

  console.log(total);

};
calibrateNumbers(input);